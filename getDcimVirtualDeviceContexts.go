  ...
  GET a list of device context objects`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if responseObject.Count > 0 {
			color.Cyan("\n  Total Metropolis Virtual Device Contexts: "+color.YellowString("%d"), responseObject.Count)

			for _, result := range responseObject.Results {
				display := fmt.Sprintf("    Metropolis Virtual Device Context: %s", color.YellowString(result.Display))
			...
			color.Cyan("  Metropolis Virtual Device Contexts: " + color.RedString("No virtual device context found on server. Exiting...\n"))
		}
	},
}
