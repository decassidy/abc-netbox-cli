  ...
  GET a list of virtual chassis objects`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if responseObject.Count > 0 {
			color.Cyan("\n  Total Metropolis Virtual Chassis: "+color.YellowString("%d"), responseObject.Count)

			for _, result := range responseObject.Results {
				display := fmt.Sprintf("    Metropolis Virtual Chassis: %s", color.YellowString(result.Display))
			...
			color.Cyan("  Metropolis Virtual Chassis: " + color.RedString("No virtual chassis found on server. Exiting...\n"))
		}
	},
}
